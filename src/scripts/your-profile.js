...
>>>>>>> upstream/dev
  ...
  if (userIsfollow) {
    document
      .querySelector("#your-profile-wrap .M-button")
      .classList.add("followed");
    document.querySelector("#your-profile-wrap .M-button.followed").innerText =
      "언팔로우";
  }
  return userIsfollow;
}
...
async function getMyProduct() {
  ...
  try {
    ...
    if (res.status == 401) {
      ...
      alert(
        "상품을 불러오는 도중 에러가 발생했습니다. 서버관리자에게 문의하거나 잠시 후 다시 시도해주세요"
      );
...
>>>>>>> upstream/dev
        ...
        document.querySelector(".post-album-view").innerHTML += `
                <li class="post-album-item"><img src="${
                  postImage[0]
                }" alt="사용자 업로드 이미지" class="upload-img-album"><img src="../../img/iccon-img-layers.svg" alt="${
          postImage.length - 1
        }장의 사진 더보기" class="icon-img-layers"></li>
                `;
      } else {
  ...
  if (homePost[0].className === "home-post") {
    homePost.forEach((element) => {
      ...
      element.style.height = `${
        parseInt(
          getComputedStyle(document.querySelector(".post-album-view")).height
        ) + 130
      }px`;
...
<<<<<<< HEAD
  ...
  for (let postIndex = 0; postIndex < takeOutPost.length; postIndex++) {
    ...
    document
      .querySelector(`.post-wrap._data${postIndex}`)
      .setAttribute("postId", takeOutPost[postIndex].id);
    document
      .querySelector(`.post-wrap._data${postIndex} .post-comment`)
    ...
    if (postImage[0] !== "") {
      for (let i = 0; i < postImage.length; i++) {
        ...
        let imgCont = document.querySelector(
          `._data${postIndex} .post-img-container`
        );
        ...
        setTimeout(() => {
          ...
          let imgBtnCont = document.querySelector(
            `._data${postIndex} .post-img-btn-container`
          );
          ...
          imgBtn.addEventListener("click", (e) => {
            ...
            e.currentTarget.parentElement.previousElementSibling.style.transitionDuration =
              ".3s";
            e.currentTarget.parentElement.previousElementSibling.scrollTo({
              left:
                parseInt(
                  getComputedStyle(e.currentTarget.parentElement.parentElement)
                    .width
                ) * i,
              behavior: "smooth",
            });
          ...
          document
            .querySelectorAll(".post-img-btn:first-child")
            .forEach((element) => {
...
>>>>>>> upstream/dev
  ...
  function toggleModal(e) {
    if (
      e.currentTarget ===
      document.querySelectorAll(".post-more-vertical")[e.currentTarget.index]
    ) {
      idBox.pop();
      idBox.push(
        document
          .querySelector(`._data${e.currentTarget.index}`)
          .getAttribute("postId")
      );
    }
    ...
    if (!overlayCheck) {
      ...
      if (
        ...
        document.querySelector(".icon-post-modal ul").children[1].innerText =
          "신고";
        document.querySelector(".delete-alert p").innerText =
          "게시글을 신고할까요?";
  ...
  // 하단 모달 토글
  let iconPostFirstBtn = document.querySelector(
    ".icon-post-modal ul li:nth-child(1)"
  );
  let iconPostSecondBtn = document.querySelector(
    ".icon-post-modal ul li:nth-child(2)"
  );
...
async function getComment(e) {
  localStorage.setItem(
    "postId",
    e.currentTarget.parentElement.parentElement.getAttribute("postId")
  );
...
async function follow(e) {
  ...
  try {
    const res = await fetch(
      url + `/profile/` + localStorage.getItem("yourAccountId") + `/follow`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${myToken}`,
    ...
    document
      .querySelector("#your-profile-wrap .M-button")
      .classList.add("followed");
    document.querySelector("#your-profile-wrap .M-button").innerText =
      "언팔로우";
  } catch (error) {
    console.log(res);
    console.log("오류발생,존재하지 않는 사용자입니다.");
...
async function unfollow(e) {
  ...
  try {
    const res = await fetch(
      url + `/profile/` + localStorage.getItem("yourAccountId") + `/unfollow`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${myToken}`,
