...
async function getProfile() {
  try {
    const res = await fetch(
      `http://146.56.183.55:5050/profile/${localStorage.getItem(
        "accountname"
      )}`,
      {
        method: "GET",
...
async function getMyPost() {
  const res = await fetch(
    `http://146.56.183.55:5050/post/${localStorage.getItem(
      "accountname"
    )}/userpost`,
    {
      method: "GET",
  ...
  console.log("myPostObjectㄴㅇㄹㄴㄹㅇ", myPost);
...
// 로그아웃 모달 토글
let iconPostLogOut = document.querySelector(
      ...
      for (let i = 0; i < postImage.length; i++) {
        ...
        imgBtn.addEventListener("click", (e) => {
          ...
          e.currentTarget.parentElement.previousElementSibling.style.transitionDuration =
            ".3s";
          e.currentTarget.parentElement.previousElementSibling.style.right = `${
            302 * i
          }px`;
        });
